...
// Owner has full access (highest tier)
...
// Admin has management access (can do everything except see financial totals)
...
// Technician+ access (technician, admin, owner)
export const technicianProcedure = t.procedure.use(
  t.middleware(async opts => {
    const { ctx, next } = opts;

    if (!ctx.user || !['technician', 'admin', 'owner'].includes(ctx.user.role)) {
...
// Customer+ access (customer, technician, admin, owner)
export const customerProcedure = t.procedure.use(
  t.middleware(async opts => {
    const { ctx, next } = opts;

    if (!ctx.user || !['customer', 'technician', 'admin', 'owner'].includes(ctx.user.role)) {
